...
export function PrivacyGate({
  ...
  // Consume the old client-side bypass marker, but require a server unlock token.
  useEffect(() => {
    consumeMeetingFresh(meetingId);
  }, [meetingId]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      Promise.all([getMeeting(meetingId, unlockToken), getTranscript(meetingId, unlockToken)])
  ...
  }, [unlocked, meeting, meetingId, unlockToken]);
  ...
  async function handleUnlock(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const result = await verifyPassword(password, meetingId);
      if (result.valid && result.unlock_token) {
        setPassword("");
